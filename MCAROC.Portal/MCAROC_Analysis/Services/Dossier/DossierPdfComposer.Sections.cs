...
public partial class DossierPdfComposer
{
    ...
    private void ComposeSnapshot(IContainer container) => container.Column(col =>
    {
        ...
        var priority = EffectivePriority?.ToString().ToUpperInvariant() ?? "NOT ASSESSED";
    ...
    private void ComposeCoverageSummary(ColumnDescriptor col)
    {
        ...
        var notes = EffectiveNotAssessed;
    ...
    private void ComposeSection1(IContainer container) => container.Column(col =>
    {
        ...
        col.Item().Element(c => Lead(c,
            "A synthesised read across Corporate, Financials, Charges & Security, Compliance" +
            (ShowLitigation ? " and Litigation" : "") + ". Every flag and its severity are generated by " +
            "Cubictree's deterministic rule engine; the narrative below each flag and the cross-section " +
            "read are generated by CT AI, validated against those same findings before publication (see " +
            "Methodology note). Figures in ₹ Crore unless stated."));
            ...
            .BorderLeft(2.5f).BorderColor(DossierTheme.Maroon).Padding(11).Text(t =>
        {
            ...
            t.Span($"{EffectiveCriticalCount} Critical").FontColor(DossierTheme.MaroonDeep).SemiBold();
            t.Span("   ·   ");
            t.Span($"{EffectiveReviewCount} Review").FontColor(DossierTheme.Amber).SemiBold();
            t.Span("   ·   ");
            t.Span($"{EffectiveWatchCount} Watch").FontColor(DossierTheme.InkSoft);
            t.Span("   ·   ");
            t.Span($"{EffectivePositiveCount} Positive").FontColor(DossierTheme.Sage);
            t.Span("        Review Priority: ");
            t.Span(EffectivePriority?.ToString() ?? "Not assessed").SemiBold();
        });
        ...
        if (EffectiveStructuredSummary is { } s)
        {
        ...
        col.Item().PaddingTop(14).Text(t =>
        {
            ...
            t.Span("Flags and their severity are generated by rule-based conditions applied to the indexed " +
                "source records (e.g. Case Category = \"Insolvency\" AND Case Status = \"Pending\"; Total " +
                "Equity < ₹0 Cr) — never by CT AI. Each flag's narrative and the cross-section read above are " +
                "generated by CT AI, checked against the same finding data before publication: no number " +
                "not already present in a finding may appear, and no severity or priority may be changed. " +
