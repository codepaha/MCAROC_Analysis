    ...
    ILogger<FinalizationRecoveryService> logger)
{
    ...
    {
        var session = await db.LargeArchiveUploadSessions.FirstOrDefaultAsync(s => s.SessionId == sessionId, ct);
        if (session is null)
        ...
        // 1. Atomically claim Verifying state with fresh FinalizationAttemptId
        ...
        var claimed = await db.LargeArchiveUploadSessions
            .Where(s => s.SessionId == sessionId
                && (s.Status == LargeArchiveUploadSessionStatus.Uploading
                    || (s.Status == LargeArchiveUploadSessionStatus.Verifying && (s.FinalizationAttemptId == null || s.ActiveFinalizationExpiresUtc < DateTime.UtcNow))))
            .ExecuteUpdateAsync(u => u
                .SetProperty(s => s.Status, LargeArchiveUploadSessionStatus.Verifying)
        ...
        try
        {
            ...
            // 5. Commit RequestDocument and McaFilingBatch entities
            var docId = await CommitDocumentAndBatchAsync(session, attemptId, ct);

            return new FinalizeResult(true, null, 200, docId, session.CreatedBatchId);
        }
        catch (Exception ex)
        {
            cts.Cancel();
            ...
            session.Status = LargeArchiveUploadSessionStatus.Failed;
    ...
    private async Task ExecuteFencedMoveAsync(LargeArchiveUploadSession session, Guid attemptId, CancellationToken ct)
    {
        ...
        try
        {
            // Acquire SQL Server sp_getapplock
            ...
            // Update session status to ArchiveMoved under the lock
            await db.LargeArchiveUploadSessions
                .Where(s => s.SessionId == session.SessionId && s.FinalizationAttemptId == attemptId)
                .ExecuteUpdateAsync(u => u
    ...
    private async Task<long> CommitDocumentAndBatchAsync(LargeArchiveUploadSession session, Guid attemptId, CancellationToken ct)
    {
        var existingDoc = await db.RequestDocuments.FirstOrDefaultAsync(d => d.UploadSessionId == session.SessionId, ct);
        if (existingDoc is null)
        {
            ...
            existingDoc = new RequestDocument
            {
                ...
                IsActiveSource = false, // Archive is tracked via McaFilingBatch, not workbook active source
        ...
        var existingBatch = await db.McaFilingBatches.FirstOrDefaultAsync(b => b.UploadSessionId == session.SessionId, ct);
        ...
        // Enqueue to background processing
    ...
    public async Task<int> ReconcileIncompleteFinalizationsAsync(CancellationToken ct = default)
    {
        ...
        foreach (var sessionId in staleSessions)
        {
            try
            {
                ...
                var session = await db.LargeArchiveUploadSessions.FirstAsync(s => s.SessionId == sessionId, ct);
